...
export const useHandleStatusBoard = () => {
  const { data: consultingAppStates, isLoading: isConsultingAppStatesLoading } = useGetConsultingAppStateQuery();
  const { data: consultingAppStatesAll, isLoading: isConsultingAppStatesAllLoading } =
  ...
  const developers = useMemo(() => {
    return Array.from(new Set(filteredConsultingAppStatesAll?.map((appState) => appState.developer)));
  }, [filteredConsultingAppStatesAll]);

  return { filteredConsultingAppStates, filteredConsultingAppStatesAll, isStatesLoading, developers };
};
