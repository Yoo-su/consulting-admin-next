...
const BasicBoardAll = () => {
  ...
  const groupedStatesAll = useMemo(() => {
    return getGroupedData(filteredConsultingAppStatesAll ?? [], 'currentState', CURRENT_STATES);
  ...
  const onDragEnd = useCallback(
    (result: DropResult) => {
      ...
      const sourceList = groupedStatesAll[source.droppableId as CurrentState];
      const destinationList = groupedStatesAll[destination.droppableId as CurrentState];
  ...
  if (!filteredConsultingAppStatesAll?.length) return <EmptyBox text={'데이터가 없습니다'} />;
          ...
          {Object.values(STATE_BOARD_DOMAIN_ITEMS).map((item) => (
            <Grid item key={item.title} xs={6} md={2} lg={2} xl={2}>
              <StateCol
                ...
                groupedStates={groupedStatesAll[item.key as CurrentState] ?? []}
                title={item.title}
                color={item.color}
