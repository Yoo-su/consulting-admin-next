...
import { useHandleHoverState, useOutsideClick, usePopover } from '@/shared/hooks';
...
export const BrowserFile = memo(({ name, path, lastModified, contentType }: BrowserFileProps) => {
  const filePopover = usePopover();
  const [originalFilename = '', extension = ''] = name.split(/\.(?=[^.]*$)/);
  const { isHovered, setHoverStateTrue, setHoverStateFalse } = useHandleHoverState();
  const { GridItemWrapper, BrowserItemWrapper, BrowserInfoArea, isBasic } = useItemStyle();
    ...
    <GridItemWrapper>
      <Tooltip title={originalFilename} open={isHovered} followCursor>
        ...
        >
          ...
          <BrowserInfoArea>
            {isEditMode ? (
              <FileNameInput path={path} extension={extension} originalFileName={originalFilename} />
            ) : (
