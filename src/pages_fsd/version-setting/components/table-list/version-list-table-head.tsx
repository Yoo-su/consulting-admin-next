...
import { ArrowIconButton } from '../misc';

type VersionListTableHeadProps = {
  dispatch: Dispatch<ActionType>;
};
export const VersionListTableHead = memo(({ dispatch }: VersionListTableHeadProps) => {
  const handleClick = (event: MouseEvent<HTMLButtonElement>) => {
    const [_, arrowDirection] = event.currentTarget.id.split('-');
    const type = arrowDirection === 'up' ? 'ADD_ALL_VERSION' : 'SUB_ALL_VERSION';
    dispatch({ type });
  };
  return (
    <TableHead>
        ...
        <TableCell align="center" sx={{ fontWeight: 'bold', ...TableCellClass }}>
          <Stack direction={'row'} spacing={1} alignItems={'center'} justifyContent={'end'}>
            ...
            {allArrowButtonList.map((button, index) => (
              <ArrowIconButton key={index} handleClick={handleClick} {...button} />
            ))}
          </Stack>
        </TableCell>
      </TableRow>
