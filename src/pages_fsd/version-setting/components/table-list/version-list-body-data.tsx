...
import { arrowButtonList, TableBodyClass, TableCellClass, TableRowClass } from '../../constants';
...
export const VersionListBodyData = ({ editedList, dispatch }: VersionListDataProps) => {
  ...
  return (
    <TableBody sx={TableBodyClass}>
      {editedList?.map((version) => {
        return (
          <TableRow key={version.TableName} sx={TableRowClass(editedList.length)}>
            ...
            <TableCell sx={TableCellClass} align="right">
              <Stack direction={'row'} alignItems={'center'} justifyContent={'end'} spacing={1}>
                ...
                <ArrowIconButton handleClick={handleClick} {...arrowButtonList['up']} id={`${version.TableName}-up`} />
              </Stack>
            </TableCell>
          </TableRow>
        );
