...
export const EditSetting = ({ settingList, path, isDisabled }: EditSettingProps) => {
  if (!settingList) return null;

  const spacing = Array.isArray(settingList) && settingList[0]?.level > 1 ? 1 : 5;
    ...
    <>
      {
        <Stack spacing={spacing}>
          {Array.isArray(settingList) ? (
            <BasicFormArray settingList={settingList} path={path} isDisabled={isDisabled} />
          ) : (
