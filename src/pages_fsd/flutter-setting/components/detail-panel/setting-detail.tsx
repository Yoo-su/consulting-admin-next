...
export const SettingDetail = ({ filteredList: filteredSettingList, isDisabled }: SettingDetailProps) => {
  ...
  const settingList = subSubCategory ? grandFilteredList : subCategory ? filteredList : children;

  const path = subCategory ? [index, 'children', settingList?.Index ?? 0] : [index];

  return (
    <Stack spacing={2} sx={DetailClass}>
      <DetailTitle isTitle={filteredSettingList.length > 0} category={category} description={description} />
