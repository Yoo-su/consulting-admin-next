...
export type DetailPageSettingContextValue = {
  isPending: boolean;
...
export const DetailPageSettingContext = createContext<
  DetailPageSettingContextValue | undefined
>(undefined);

export type DetailPageSettingProviderProps = {
  children: ReactNode;
};

export const DetailPageSettingProvider = ({
  children,
  ...
  const addNewDetailpageRow = useCallback(() => {
    queryClient.setQueryData(
      QUERY_KEYS['detail-page-setting']['data'](currentService?.serviceID ?? '')
        .queryKey,
  ...
  const updateRowMode = useCallback(
    (rowNum: number, newMode: 'detail' | 'calc') => {
      queryClient.setQueryData(
        QUERY_KEYS['detail-page-setting']['data'](
          currentService?.serviceID ?? ''
        ).queryKey,
        (oldData: DetailPageData[]) => {
  ...
  return (
    <DetailPageSettingContext.Provider
      value={{
        detailpageData: data,
        hasChanges,
        isPending,
