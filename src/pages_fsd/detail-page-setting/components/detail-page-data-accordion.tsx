...
export const DetailPageDataAccordion = memo(({ rowNum, condition, mode, htmlCard }: DetailPageDataAccordionProps) => {
  const { isSelected, handleClickTitle } = useDataAccordion(rowNum);
  const { handleDeleteData, handleUpdateHtmlCard, handleUpdateMode } = useHandleDetailPageData();
  const { anchorRef, handleOpen, open, handleClose } = usePopover<HTMLDivElement>();
      ...
      <AccordionSummary>
        ...
        {isSelected && (
          <Stack direction={'row'} sx={{ ml: 1, justifyContent: 'flex-end', alignItems: 'center' }}>
            <DeleteDataButton rowNumber={rowNum} handleDeleteData={handleDeleteData} />
          </Stack>
        )}
      </AccordionSummary>
      <AccordionDetails>
        <Stack direction={'column'}>
          <Stack direction={'row'} justifyContent={'flex-start'} alignItems={'flex-end'} spacing={2}>
            <Stack direction={'row'} spacing={4} alignItems={'flex-end'}>
              <ModeSelect rowNumber={rowNum} currentMode={mode} handleModeChange={handleUpdateMode} />
              <PopoverToggler anchorRef={anchorRef} open={open} handleOpen={handleOpen} />
          ...
          <Stack direction={'column'} spacing={1}>
            ...
            <TiptapEditor value={htmlCard} handleChangeValue={handleUpdateTiptapValue} />
          </Stack>
        </Stack>
      </AccordionDetails>
    </Accordion>
