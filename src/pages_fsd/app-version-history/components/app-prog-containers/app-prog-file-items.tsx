...
export const AppProgFileItems = ({ history, setOpen }: AppProgFileItemsProps) => {
  ...
  return (
    <Grid key={history.uploadTime} item xs={12} sm={6} md={3} lg={3} xl={3}>
      <FileItemCard tooltipMsg={'다운로드 링크 복사'} handleClick={() => handleClickCard(getAppDownloadUrl(history))}>
        <FileItemCard.IconBox file={history.osType === OS_TYPE.APK ? 'apk' : 'exe'} />
